...
if (isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($resultat -> rowCount() > 0) { // signifie que le produit existe
        $cat = $resultat -> fetch(PDO::FETCH_ASSOC);
        //debug($produit);

        // Supprimer la photo du serveur :
        // $chemin_photo_a_supprimer = $_SERVER['DOCUMENT_ROOT'] . RACINE_SITE . 'photo/' . $annonce['photo'];
        ...
        $resultat = $pdo -> exec("DELETE FROM categorie WHERE id_categorie = $cat[id_categorie]");

        if($resultat) {
            header('location:gestion_des_categories.php?msg=suppr&id=' . $cat['id_categorie']);
        }
    }
}
